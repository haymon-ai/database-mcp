...
- **`crates/pii/`** (`dbmcp-pii`) — Regex-based PII detection (32 built-in entity types covering personal, financial, government, contact, network, digital-identity, and crypto categories) tagged by `Category` (`Personal`, `Financial`, `Government`, `Contact`, `Network`, `DigitalIdentity`, `Crypto`), four anonymisation operators (`replace`, `mask`, `redact`, `hash`), and an `Analyzer::builder()` entry point for category-filtered registries. Wired into query tool output behind `PiiConfig`. Canonical entity list: `crates/pii/src/entity.rs` (`Entity` enum); registration order: `crates/pii/src/recognizers/mod.rs` (`all()`).
