...
pub fn id_card_de() -> Recognizer {
    ...
    Recognizer::new(Entity::IdCardDe, patterns)
        ...
        .with_name("IdCardDeRecognizer")
        .with_validator(Validator::DeIdCard)
        .with_category(Category::Government)
}

#[cfg(test)]
mod tests {
    use super::id_card_de;

    fn matches(text: &str) -> Vec<(usize, usize)> {
        let mut spans: Vec<(usize, usize)> = id_card_de()
            .analyze(text)
            .into_iter()
            .map(|r| (r.start, r.end))
            .collect();
