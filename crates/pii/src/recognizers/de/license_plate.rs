...
#[must_use]
pub fn license_plate_de() -> Recognizer {
    let patterns = vec![
        Pattern::new(
    ...
    Recognizer::new(Entity::LicensePlateDe, patterns)
        ...
        .with_name("LicensePlateDeRecognizer")
        .with_category(Category::Government)
}

#[cfg(test)]
mod tests {
    use super::license_plate_de;

    fn matches(text: &str) -> Vec<(usize, usize)> {
        let mut spans: Vec<(usize, usize)> = license_plate_de()
            .analyze(text)
            .into_iter()
            .map(|r| (r.start, r.end))
            .collect();
    ...
    #[test]
    fn recognizes_license_plate_de() {
