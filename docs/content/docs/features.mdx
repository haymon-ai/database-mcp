...
List tables in a specific database, one page at a time. Requires a database name (use `listDatabases` first on MySQL/MariaDB/PostgreSQL). Results are paginated — see [Cursor Pagination](#cursor-pagination) for the protocol.
