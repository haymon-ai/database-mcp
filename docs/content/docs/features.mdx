...
Eight built-in regex recognisers ship by default. Each recogniser is identified by an entity type that drives the `replace` operator's placeholder, and is tagged with a category for opt-in subsetting via `--pii-categories` / `PII_CATEGORIES`:
...
- `crypto` — `CRYPTO`
- `digital-identity` — reserved; no built-in recogniser tags this category today

See [PII configuration](/docs/configuration#pii) for the flag, env var, and the empty-registry fallback rules.
