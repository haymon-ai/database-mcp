...
The page size applies uniformly to every paginated tool; see the [Cursor Pagination](/docs/features#cursor-pagination) section on the Features page for the full protocol.
...
- `crypto` — `CRYPTO`
- `digital-identity` — *(no built-in recogniser tags this category today; reserved for future additions)*

An empty list is rejected at startup. If the selected categories produce an empty registry (for example `digital-identity` alone), the analyzer falls back to the default registry and logs a warning so redaction is never silently disabled.

> **Default**: redaction is off. Setting `PII_ENABLE=true` (or `--pii true`) is the only way to activate the redactor — `PII_OPERATOR` and `PII_CATEGORIES` alone have no effect.
